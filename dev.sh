...
_sparkrun_sparkroute_dev_setup() {
    ...
    if [[ -n "${SPARKROUTE_BINARY:-}" && \
          "${SPARKROUTE_BINARY}" != "${_SPARKRUN_SPARKROUTE_MANAGED_BINARY:-}" ]]; then
        if [[ ! -f "$SPARKROUTE_BINARY" || ! -x "$SPARKROUTE_BINARY" ]]; then
            echo "SPARKROUTE_BINARY must name an executable file: $SPARKROUTE_BINARY" >&2
            return 1
        fi
        echo "Using explicit SparkRoute development binary: $SPARKROUTE_BINARY"
        ...
        export SPARKROUTE_BINARY="$gateway_binary"
